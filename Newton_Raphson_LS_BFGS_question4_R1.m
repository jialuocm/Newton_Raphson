...
for cc = 1:2
    ...
    for n = 1: n_step
        ...
        a0 = 1; d = d + a0*delta_d; %First iteration use Newton update,new guess
        %--------------------------------------------
        % iteration loop
        while (er>stop_tol && count<maxit)
            ...
            % Do Line search if it is necessary ,solve line search scale s
             ...
             rs = r(d,f_t,const);        %calculate the residual
            % -------------------------------------------------------------------------
            % 2.compute the search direction by multiple BFGS update algorithm.
            % -------------------------------------------------------------------------
            % delta_d = findnewd(delta_d,Ji,r0,rs,G0,G,s,count);
             [delta_d] = findnewd(delta_d,Ji,r0,rs,s,count);
             r0 = rs;
            % -------------------------------------------------------------------------
            % 1. Main line search loop. iterate to find search parameter s.
            % -------------------------------------------------------------------------       
            % Find s s.t. delta_d is orthogonal to residual vector R,
            % or the potential energy functional is minimized.
            G0 = dot(delta_d,r0);
            a0 = 1; d_temp = d + a0*delta_d; 
